...
export default {
  ...
  getHomeTest: asyncHandler(async (req, res, next) => {
    ...
    if (!homeTest) {
      return next(
        new ErrorResponse(
          `No homeTest found with the id of ${req.params.id}`,
          404
        )
      )
    }
  ...
  requestHomeTest: asyncHandler(async (req, res, next) => {
    ...
    if (!lab) {
      return next(
        new ErrorResponse(`No lab with the id of ${req.params.labId}`, 404)
      )
    }
  ...
  respondHomeTest: asyncHandler(async (req, res, next) => {
    ...
    if (!homeTest) {
      return next(
        new ErrorResponse(
          `No homeTest with the id of ${req.params.homeTestId}`,
          404
        )
      )
    }
    if (homeTest.lab.toString() !== req.params.labId) {
      return next(
        new ErrorResponse(
          `No appointment with the id of ${req.params.id} belong to this lab`,
          404
        )
      )
    }

    homeTest = await HomeTest.findByIdAndUpdate(
      req.params.homeTestId,
      req.body,
      {
        new: true,
        runValidators: true
  ...
  deleteHomeTest: asyncHandler(async (req, res, next) => {
    ...
    if (
      homeTests.doctor.toString() !== req.user.id &&
      req.user.role !== 'admin'
    ) {
