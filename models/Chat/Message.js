 ...
 */
MessageSchema.statics.createPostInChatRoom = async function (chatRoomId, message, postedByUser ,role) {
  try {
 ...
 * @param {String} role - user role 
 */
MessageSchema.statics.getConversationByRoomId = async function (chatRoomId, options = {} , role) {
  try {
    return this.aggregate([
      { $match: { chatRoomId } },
      { $sort: { createdAt: -1 } },
 ...
 * @param {String} role - user role 
 */
MessageSchema.statics.getRecentConversation = async function (chatRoomIds, options, currentUserOnlineId , role) {
  try {
    if(role === "user"){
    return this.aggregate([
